前面我们分别学习了事件循环、Future 和协程的底层原理。从这节课开始，我们不再自己造轮子，而是使用 Python 官方和社区给我们准备好的 API，高效地进行异步开发。

| Python                                   | JavaScript 对应                      |
| ---------------------------------------- | ------------------------------------ |
| 异步生成器 / `async for`                 | `async function*` / `for await...of` |
| `__aiter__` / `__anext__`                | `[Symbol.asyncIterator]`             |
| `async with` / `__aenter__`              | 无直接对应（常见 `try` / `finally`） |
| `asyncio.sleep`                          | `Promise` + `setTimeout`             |
| `asyncio.run`                            | 顶层 `await` / 入口处消费 Promise    |
| `asyncio.create_task`                    | 直接调用 async 函数（立即开始）      |
| `asyncio.gather`                         | `Promise.all`                        |
| `asyncio.wait`                           | `Promise.race` 等（部分近似）        |
| `asyncio.as_completed`                   | 无直接对应                           |
| `asyncio.TaskGroup`                      | 无直接对应                           |
| `Lock` / `Event` / `Semaphore` / `Queue` | 无内置                               |
| `wait_for` / `timeout`                   | `AbortSignal.timeout` / 超时竞态     |
| `to_thread` / `run_in_executor`          | 见 Node.js（工作线程）               |
...
const main = async () => {
  for await (const num of new AsyncCounter(5)) {
    console.log(num); // 每隔 0.5 秒打印 1 2 3 4 5
  }
};
```
...
**无 JavaScript 对应。** JavaScript 没有与 `__aenter__` / `__aexit__` + `async with` 完整对等的协议；业务上常用 `try` / `finally` 自行保证释放。较新的 `await using` 只覆盖释放侧（`Symbol.asyncDispose`），进入侧仍需自行编排。
...
const main = async () => {
  console.log("开始");
  await sleep(1000); // 挂起当前异步函数 1 秒，事件循环去调度其他任务
  console.log("1秒后");
};
```
...
> **说明：** `asyncio.run` 会创建并关闭事件循环；JavaScript 运行时通常已有内置事件循环，入口处 `await`（或 `.then`）即可。
...
> **说明：** Python 调用协程函数只得到未启动的协程对象，需 `create_task` / `asyncio.run` 等驱动；JavaScript 调用 async 函数会立刻开始执行并返回 Promise。
...
const fetchUrl = async (url, delay) => {
...
const main = async () => {
  ...
  console.log(results); // ['url1 完成', 'url2 完成', 'url3 完成']
};
...
- 所有协程**并发**执行
- 返回结果顺序与传入顺序一致
- 任何一个协程抛出异常，gather 会立刻传播异常（其他协程仍会继续运行）
...
JavaScript 类比（对应 `return_exceptions=True`）：
...
const main = async () => {
  const results = await Promise.allSettled([fetchUrl("ok", 1), fail()]);
  console.log(results);
  // [{ status: 'fulfilled', value: 'ok 完成' }, { status: 'rejected', reason: Error }]
};
```

> **说明：** `Promise.allSettled` 不会因单个失败而整组抛错，语义接近 `return_exceptions=True`；默认的 `Promise.all` 更接近 gather 默认「任一失败即 reject」。
...
JavaScript 类比（仅近似 `FIRST_COMPLETED`）：

```js
const main = async () => {
  const tasks = [fetchUrl("A", 2), fetchUrl("B", 1), fetchUrl("C", 3)];

  // Promise.race：任一完成（或失败）就返回该结果
  const first = await Promise.race(tasks);
  console.log(first);
};
```

> **说明：** `Promise.race` 只返回第一个 settled 的值，不会像 `asyncio.wait` 那样拆出 `done` / `pending` 集合，也没有 `FIRST_EXCEPTION` / `ALL_COMPLETED` 等内置模式。
...
**无 JavaScript 对应。** 标准库没有「按完成先后依次产出」的内置迭代器；需要自行用 `Promise.race` 对剩余任务轮询，或借助第三方工具。
...
**无 JavaScript 对应。** 标准 Promise API 没有「组内任一失败则取消其余任务」的结构化并发原语；业务上常靠 `AbortController` 自行编排取消，语义需自己保证。
...
**无 JavaScript 对应。** 语言标准库没有与 `asyncio.Lock` 对等的异步互斥锁；单线程事件循环里多数共享状态仍需自行约定临界区，或使用第三方实现。
...
**无 JavaScript 对应。** 标准库没有内置的「可多次等待 / 广播唤醒」的 `Event`；常见用共享 Promise、或自行封装 resolve 列表来模拟。
...
**无 JavaScript 对应。** 标准库没有内置异步信号量；限制并发通常自行用计数 + 等待队列，或使用第三方库。
...
**无 JavaScript 对应。** 标准库没有带背压（`maxsize`）与 `task_done` 语义的异步队列；需自行实现或使用第三方库。
...
const main = async () => {
  try {
    ...
    console.log(e.message); // 操作超时了
  }
};
```

> **说明：** 上面是「超时竞态」近似；较新的运行时也可用 `AbortSignal.timeout(ms)` 配合可取消的底层操作。`wait_for` 超时后会取消被等待的任务，简单 `Promise.race` 不会自动取消另一方。
...
**无 JavaScript 对应。** 没有与 `async with asyncio.timeout(...)` 对等的超时上下文管理器；超时通常写成 `Promise.race` / `AbortSignal.timeout`，而不是块级 `async with`。
...
if (isMainThread) {
  const runInWorker = (fnSource, data) =>
    new Promise((resolve, reject) => {
      const worker = new Worker(fnSource, { eval: true, workerData: data });
  ...
  const result = await runInWorker(
    ...
    let sum = 0
    for (let i = 0; i < workerData; i++) sum += i * i
  ...
  // worker 侧由上面的 eval 脚本处理
}
```

> **说明：** `asyncio.to_thread` / `run_in_executor` 把阻塞工作丢到线程池，避免卡住事件循环；Node.js 主线程同样不能跑重 CPU / 同步阻塞，常见做法是 `worker_threads`（或把 I/O 换成异步 API，如 `fs.promises`）。
...
> **说明：** `httpx` 同时提供同步 `Client` 与异步 `AsyncClient`；浏览器 / 现代运行时的 `fetch` 本身是异步的，同步 HTTP 需另寻方案。
...
const readWriteExample = async () => {
  await writeFile("example.txt", "Hello, 异步文件!\n", "utf8");
...
const batchProcess = async (urls) => {
  ...
  return await Promise.all(urls.map((url) => process(url)));
};
```
...
JavaScript 类比（自行实现信号量 + 延时释放）：

```js
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

class RateLimiter {
  constructor(maxRate, interval = 1000) {
    this._tokens = maxRate;
...
const main = async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 6 }, (_, i) => fetchUrl(`url${i}`)),
  );
  console.log(results); // 每秒最多完成 2 个请求
};

main();
```

> **说明：** JavaScript 没有内置 `Semaphore` / `async with`，上面用计数 + 等待队列近似同一限速思路。
...
const retry = async (factory, maxRetries = 3, delay = 1000) => {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      return await factory();
    } catch (e) {
      if (attempt === maxRetries - 1) throw e;
      console.log(`第 ${attempt + 1} 次失败，${delay / 1000} 秒后重试...`);
      await sleep(delay);
    }
  }
};
...
const main = async () => {
  ...
  const unstableRequest = async () => {
    n += 1;
    if (n < 3) throw new Error(`第 ${n} 次请求失败`);
    return "成功响应";
  };

  const result = await retry(unstableRequest, 3, 500);
  console.log(result); // 前 2 次失败，第 3 次成功
};
...
const bad = async () => {
  // 错误示意：同步忙等 / 同步阻塞会卡住事件循环
  const end = Date.now() + 1000;
  while (Date.now() < end) {}
};

const good = async () => {
  await sleep(1000); // 正确：主动让出控制权
};
```

> **说明：** 上例只对应「不要用同步忙等卡住事件循环」；把阻塞丢到线程池的 `asyncio.to_thread` 见前文 Node.js（`worker_threads`）一节。
...
```js
// 通常无需手写“创建 / 绑定 / run_forever”事件循环
// 入口处直接消费 Promise 即可
await main();
// 或：main().catch(console.error)
```
...
```js
const main = async () => {
  // 注意：JavaScript 调用 async 函数会立刻开始执行
  // “创建了却完全不跑”在 Python 更常见；这里更常见的是忘记 await 导致竞态 / 未处理 rejection
  fetchUrl("url", 1); // 已启动，但未 await，错误可能变成 unhandled rejection

  await fetchUrl("url", 1);

  await Promise.all([fetchUrl("url1", 1), fetchUrl("url2", 2)]);
};
```

> **说明：** Python 忘记 `await` 时协程对象往往根本不执行；JavaScript 忘记 `await` 时任务通常已经启动，只是没有等待结果。
...
```js
const main = async () => {
  const results = await Promise.allSettled([riskyTask(), safeTask()]);
  for (const r of results) {
    if (r.status === "rejected") {
      console.log(`某个任务失败: ${r.reason}`);
    }
  }
};
```
...
**无 JavaScript 对应。** 浏览器 / 引擎没有与 `asyncio.run(..., debug=True)` 对等的官方「异步调试开关」。
...
**无 JavaScript 对应。** 没有内置「协程卡住超过 N 秒就打印栈」的 asyncio debug 等价物。
...
**无 JavaScript 对应。** JavaScript 调用 async 函数会立刻执行，不存在「创建了协程对象却从未启动」的同款警告；更常见的是未处理的 Promise rejection。
...
**无 JavaScript 对应。** 没有与 `slow_callback_duration` 对等的事件循环慢回调阈值配置。
...
**无 JavaScript 对应。** 运行时通常不暴露可被模块顶层「取错」的全局事件循环对象；业务代码一般也不手动 `get_event_loop`。
...
JavaScript 类比（用 `AbortSignal` 近似取消）：
...
> **说明：** Python 取消会注入 `CancelledError`，清理后必须再抛出；JavaScript 没有同名异常，取消语义通常靠 `AbortSignal` / 自行约定，需在说明里区分开。

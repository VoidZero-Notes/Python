...
| 类别 | 代表方法 | 触发场景 |
| ---- | -------- | -------- |
| 字符串表示 | `__str__` / `__repr__` | `print()`、`str()`、`repr()` |
| 比较操作 | `__eq__` / `__lt__` 等 | `==`、`<`、`>` 等 |
| 算术运算 | `__add__` / `__mul__` 等 | `+`、`-`、`*`、`/` 等 |
| 容器协议 | `__len__` / `__getitem__` 等 | `len()`、`[]`、`in`、`for` |
| 类型转换 | `__int__` / `__bool__` 等 | `int()`、`float()`、`bool()` |
| 属性访问 | `__getattr__` / `__setattr__` 等 | 读 / 写 / 删属性 |
...
- `isinstance(other, Person)`：确认对方也是 `Person`。像 `alice == 30` 这种跨类型比较，应进入该分支，避免对非 `Person` 访问 `.age` 报错。
- `return NotImplemented`：不是返回 `False`，而是告诉解释器「这个类型我不会比」。Python 会再尝试对方的对应方法；两边都不行时，`==` 最终为 `False`，`<` / `>` 等则可能抛 `TypeError`。

| 写法 | 效果 |
| ---- | ---- |
| `return False` | 直接认定不相等，可能误伤（子类、合理的跨类型比较） |
| `return NotImplemented` | 按协议声明「我不会」，交给 Python 走默认逻辑，更规范 |
...
**警告：** 在 `__getattribute__` 中再次访问 `self.xxx` 也会触发自身，必须使用 `object.__getattribute__(self, name)`。
...
console.log(userProxy.age);   // get age → 18
userProxy.age = 20;           // set age 20
console.log(userProxy.age);   // get age → 20
console.log(userProxy.email); // get email → undefined
```

> **说明：** `Proxy` 的 `get` / `set` 分别近似 `__getattribute__` / `__setattr__`——读写**任意**属性都会进陷阱；不像 `__getattr__` 只在属性不存在时触发。访问不存在的属性时，JavaScript 返回 `undefined`，Python 通常抛 `AttributeError`。

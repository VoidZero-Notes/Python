...
| 概念 | 含义 |
| ---- | ---- |
| 横切关注点（Cross-Cutting Concerns） | 跨越多个功能、可被横切的共同逻辑 |
| AOP（Aspect Oriented Programming） | 把那些跨多个功能、可被横切的共同逻辑分离出去，不侵入功能本身 |
| DI | AOP 的一种实现手段 |
...
![image-20260622134925991](https://ipic-typora.oss-cn-shenzhen.aliyuncs.com/jishu-img/2026-08-15-095539.png)

本章落地三个中间件：

| 中间件 | 文件 | 作用 |
| ------ | ---- | ---- |
| `unified_response` | `response.py` | 把 `/api/` 响应包成 `{code, data, message}` |
| `process_time` | `process_time.py` | 写入 `X-Process-Time` 响应头 |
| `CORSMiddleware` | `cors.py` | 按配置处理跨域 |

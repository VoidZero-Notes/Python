DML（Data Manipulation Language，数据操纵语言）用于对表中的数据进行**增、删、改、查**。它是日常数据库操作中使用最频繁的部分，绝大多数业务代码最终都落在这四类语句上。
...
`DELETE` 属于 DML，逐行删除数据，可以带 `WHERE` 只删部分行，且不会重置自增序列；`TRUNCATE` 属于 DDL，直接重置底层存储，不能带 `WHERE`，并且会重置自增序列。
...
> **说明：** `DELETE` / `TRUNCATE` / `DROP` 三者的完整对比见《03. DDL》。
...
> OFFSET 从 0 开始计数，第 1 条的偏移量为 0。
>
> MySQL 的 `LIMIT 偏移量, 数量` 简写语法 PostgreSQL 不支持，必须写成 `LIMIT n OFFSET m`。

...
FOREIGN KEY (dept_id) REFERENCES departments(id)
  ...
  ON UPDATE CASCADE;   -- 级联更新：部门 id 更新时，员工 dept_id 自动更新
```
...
erDiagram
  ...
  product ||--o{ sku : "一对多"
  user ||--o{ order : "一对多"
  order ||--o{ order_items : "多对多（中间表）"
  sku ||--o{ order_items : "多对多（中间表）"
```

> **说明：** `order` 与 `sku` 之间是多对多，靠中间表 `order_items` 拆成两条一对多；中间表额外存了 `quantity`、`price` 这类属于「关系本身」的属性。
